...
const ManageFiles = () => {
    ...
    const handleFileChange = async (event) => {
        ...
        if (mimTypeArray.includes(event.target.files[0].type)) {
            ...
            const data = await fileUploadAPI('/upload/clientMediaFiles', fileData)
            if (data.status) {
    ...
    const getData = async () => {
        ...
        if (res.status) {
            ...
            setTotalUsed(res.totalFileSizeUsed)
        } else {
    ...
    useEffect(() => {
        const observer = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting) {
                if (data.length < dataCount) setPage((prev) => prev + 1)
            }
        }, { threshold: 0.5 });
    ...
    return (
        <div className="p-1">
            ...
            <div className='flex justify-between py-2 px-2 mt-5'>
                    ...
                    <div className="flex flex-col">
                        <div className="-m-1.5 ">
                            <div className="p-1.5 min-w-full inline-block align-middle">
                                                ...
                                                data?.length > 0 && data?.map((item, index) => {
                                                    return (
                                                        <tr className="border border-gray-200 cursor-pointer last-item" key={index}>
                                                            <td className="px-4 whitespace-nowrap text-center text-lg text-gray-800 font-medium py-3 border border-gray-200 ">{(item?.fileType === "jpeg" || item?.fileType === "png") ? <FaRegFileImage className='ml-3' /> : (item?.fileType === "pdf") ? <FaFilePdf className='ml-3 text-red-500' /> : (item?.fileType === 'mp4' || item?.fileType === 'xls') ? < IoMdVideocam className='ml-3 text-blue-400' /> : ''}</td>
                                                            <td className="px-4 whitespace-nowrap text-center text-xs text-gray-800 font-medium py-3 border border-gray-200">{item?.fileOriginalName}</td>
                                                            <td className="px-4 whitespace-nowrap text-center text-xs text-gray-800 font-medium py-3 border border-gray-200">{item?.fileNewName}</td>
                                                            <td className="px-4 whitespace-nowrap text-center text-xs text-gray-800 font-medium py-3 border border-gray-200">{item?.createdAt && moment(item?.createdAt).format("DD-MMM-YYYY hh:mm:ss A")}</td>
                                                            <td className="px-4 whitespace-nowrap text-center text-xs text-gray-800 font-medium py-3 border border-gray-200">{item?.fileSize ? formatBytes(item.fileSize) : formatBytes(0)}</td>
                                                            <td className="px-4 whitespace-nowrap text-center text-xs text-gray-800 font-medium py-3 border border-gray-200"><FaEye onClick={() => {
                                                            ...
                                                            <td className="px-4 whitespace-nowrap text-center text-xs text-gray-800 font-medium py-3 border border-gray-200"><FaRegCopy onClick={() => {
                                                            ...
                                                            <td className="px-4 whitespace-nowrap text-center text-xs text-gray-800 font-medium py-3 border border-gray-200"><FaTrash className='text-red-500 cursor-pointer self-center ml-[30%] text-lg' onClick={() => deleteFile(item?._id)} /></td>
                                                        </tr>
                                                    )
                                                })
                                            }
                                ...
                                {data?.length === 0 && <h4 className="text-center text-lg mt-5">No Data Found.</h4>}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <p className='text-xs text-black pt-2 mr-2'>{formatBytes(totalUsed)} out of 100 MB used</p>
        </div>
    )
}
